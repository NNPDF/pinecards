...
TEST_CASE("", "")
{
    ...
    int const lumi_id = applsaft_lumi_new();
    ...
    applsaft_lumi_add(lumi_id, pdg_id_pairs.size() / 2, pdg_id_pairs.data(), 1.0);
    ...
    // create a new file
    int file_id = applsaft_file_open(
        bin_limits.size() - 1,
        bin_limits.data(),
        lumi_id,
        applsaft_grid_format::as_a_logmur_logmuf,
    ...
    applsaft_file_fill(file_id, 0.25, 0.25, 10000.0, 0.25, weights.data(), 0);

    double result;

    applsaft_file_convolute(
        file_id,
        simple_pdf,
        simple_pdf,
        alphas,
