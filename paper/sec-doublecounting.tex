...
It is interesting
to note that one can tune the QED parton shower to specific decays to mimic NLO EW effects, so that a prediction only accurate at NLO
...
Another problem with this approach is that it introduces a dependence on the program that is used for the shower inversion, or even on a specific version of the program, for examples if bugs have been found.
This is especially problematic if these datasets are the only ones published, since undoing the exact subtraction can be very difficult or practically impossible as time passes (TODO insert comment about LEP ISR and DIS EW effects removal).
