...
We first describe the general scope and features of the library in comparison to \textsc{APPLgrid} and \textsc{fastNLO} in section~\ref{ref:library}.
...
\paragraph{4-tuples}
A straightforward representation is given by 4-tuples, i.e.\ a list of the momentum fractions $x_1$ and $x_2$, the scale $Q^2$, and the phase-space weight $w$ for each phase-space point; 4-tuples are sufficient to reconstruct a total cross section.
...
\begin{equation}
a_{i,j} \leftarrow a_{i,j} + I_i(u(y(x_1))) I_j(u(y(x_2))) W(x_1,x_2) / \omega(x_1,x_2)
...
In section~\ref{sec:multi-coupling-expansion} we labelled the different perturbative orders using the indices $k,l,m,n$.
Their values are process specific.
