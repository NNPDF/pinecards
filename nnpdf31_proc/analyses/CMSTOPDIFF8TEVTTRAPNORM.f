c
c Analysis for CMSTOPDIFF8TEVTTRAPNORM.
c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,3
         l=(i-1)*5
         call HwU_book(l+ 1,'t-tx pair rap '//cc(i), 1,-2.5d0,-1.3d0)
         call HwU_book(l+ 2,'t-tx pair rap '//cc(i), 1,-1.3d0,-0.9d0)
         call HwU_book(l+ 3,'t-tx pair rap '//cc(i), 6,-0.9d0,+0.9d0)
         call HwU_book(l+ 4,'t-tx pair rap '//cc(i), 1,+0.9d0,+1.3d0)
         call HwU_book(l+ 5,'t-tx pair rap '//cc(i), 1,+1.3d0,+2.5d0)
      enddo
      return
      end
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $     ,ptsyst,ysyst,etasyst,p_bjet(0:3,nexternal),pttx(0:3)
...
c      ytx = getrapidity(p(0,4), p(3,4))
      ...
      do i=1,3
         ...
         call HwU_fill(l+1,yttx,wgts)
         call HwU_fill(l+2,yttx,wgts)
         call HwU_fill(l+3,yttx,wgts)
         call HwU_fill(l+4,yttx,wgts)
         call HwU_fill(l+5,yttx,wgts)
      enddo
c
 999  return      
      end
