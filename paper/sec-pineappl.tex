...
which they are compured. Indeed, beyond LO,
different kinematic contributions have to be considered (in Ref.~\cite{Bertone:2014zva}, they are labeled by the index $\alpha$, see
Eq. 12 therein). Their consistent treatment must be ensured by the interface between
\textsc{PineAPPL} and the specific cross-section compuation at hand\footnote{
    For example, while in the FKS subtraction scheme~\cite{Frixione:1995ms,Frixione:1997np} employed in \textsc{mg5\_aMC@NLO}
    maximally one kind of kinematics for each counterterm (soft, collinear and soft-collinear) is needed, this 
    is not the general case, for example in the Catani-Seymour one~\cite{Catani:1996jh}}.
