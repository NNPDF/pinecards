...
With respect to the capabilities of these computer codes, \textsc{PineAPPL} makes it possible for the first time to include also higher-order corrections due to EW, and in general combined QCD--EW, effects.
...
All remaining corrections are of combined type, meaning that they, in general, cannot be attributed to either one of strong or electroweak origin. However, for the sake of simplicity and with a slight abuse of notation, it is customary to refer
...
To illustrate the filling step we give an example in figure~\ref{fig:grid}, where, for simplicity, we have chosen a static scale, so that we do not need to interpolate in $\tau$ direction, and where we also limited the number of grid points to $N_y = 10$.
...
% TODO: explain how convolutions are performed
