...
The advantage of this method is that the time-consuming first step has to be done only once and therefore allows for very quick convolutions of a grids with multiple PDF sets.
...
However, it supports electroweak (EW) predictions/corrections, which are the main interest in this paper.
...
\item EW corrections require the inclusion of photon-initiated contributions, which are supported as well.
In fact, \textsc{PineAPPL} allows arbitrary initial-state combinations, e.g.\ leptonic initial states, for which PDFs have been calculated recently~\cite{Buonocore:2020nai}.
\item Variations of the renormalisation and factorisation scale around the central scale choice are supported, which are needed to estimate the QCD scale uncertainty.
...
\end{itemize}
On a more technical level, we further point out the following differences of \textsc{PineAPPL}:
\begin{itemize}
\item Earlier experiments with APPLgrid (only QCD corrections) showed that it uses a substantial amount memory (close to \SI{80}{\giga\byte}), if a grid for a differential distributions with more than 10 bins was generated.
...
However, this means that this two-step procedure of 1) producing an unoptimised grid and 2) filling an optimised grid effectively limits the bins to a small number.
...
C was chosen because it plays well with Fortran and C++, in which most MC integrators are written.
The interface consists of roughly 30 functions of which basically only a handful are needed.
...
\item The program \texttt{pineappl}, which performs convolutions on the command line (see appendix~\ref{app:pineappl-demo}), without requiring the user to write a new program.
...
\end{itemize}
For \textsc{mg5\_aMC@NLO}~\cite{Alwall:2014hca,Frederix:2018nkq} the interfacing code is already implemented\footnote{It can be downloaded from \url{https://code.launchpad.net/~amcblast/+junk/3.0.2}}, which is intended to replace the \textsc{aMCfast}~\cite{Bertone:2014zva} interface.
Its usage is very similar to that of \textsc{mg5\_aMC 2}+\textsc{aMCfast}+\textsc{APPLgrid}.
However, using \textsc{PineAPPL} is much faster especially for small runs, because the grids do not need to be optimised and are merged faster.
See appendix~\ref{app:sample-runcard} for an example of a runcard.

Interfacing code for other MC generators should be easy to write, see appendix~\ref{app:example-program} for a small example program.
