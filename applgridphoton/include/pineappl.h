...
/** Type definition for a pointer to @ref pineappl_lumi. */
typedef struct pineappl_lumi* pineappl_lumi_ptr;
...
/** Type definition for a pointer to @ref pineappl_storage. */
typedef struct pineappl_storage* pineappl_storage_ptr;

/**
 * Return a pointer to newly-created @ref pineappl_storage object. The parameter method determines
 * the storage layout.
 */
pineappl_storage_ptr pineappl_storage_new(char const* method);

/** Delete the previously created object pointed to by `storage`. */
void pineappl_storage_delete(pineappl_storage_ptr storage);

/** Set the double-valued parameter with name `key` to `value` for `storage`. */
void pineappl_storage_set_bool(pineappl_storage_ptr storage, char const* key, int value);

/** Set the double-valued parameter with name `key` to `value` for `storage`. */
void pineappl_storage_set_double(pineappl_storage_ptr storage, char const* key, double value);

/** Set the int-valued parameter with name `key` to `value` for `storage`. */
void pineappl_storage_set_int(pineappl_storage_ptr storage, char const* key, int value);

/** Set the string-valued parameter with name `key` to `value` for `storage`. */
void pineappl_storage_set_string(pineappl_storage_ptr storage, char const* key, char const* value);

/** Get the boolean-valued parameter with name `key` for `storage`. */
int pineappl_storage_get_bool(pineappl_storage_ptr storage, char const* key);

/** Get the double-valued parameter with name `key` for `storage`. */
double pineappl_storage_get_double(pineappl_storage_ptr storage, char const* key);

/** Get the string-valued parameter with name `key` for `storage`. */
int pineappl_storage_get_int(pineappl_storage_ptr storage, char const* key);

/** Get the int-valued parameter with name `key` for `storage`. */
char const* pineappl_storage_get_string(pineappl_storage_ptr storage, char const* key);
...
/** Type definition for a pointer to @ref pineappl_grid. */
typedef struct pineappl_grid* pineappl_grid_ptr;
 ...
 * - Subgrid specification: `format`, `subgrids`, and `subgrids_params`. Each PineAPPL grid contains
 *   a number of subgrids, given as `subgrids`, which usually store the grids for each perturbative
 *   order separately. The concrete meaning of the subgrids is determined by `format` and
 *   `subgrids_params`.
 ...
 */
pineappl_grid_ptr pineappl_grid_new(
    pineappl_lumi_ptr lumi,
    ...
    pineappl_storage_ptr storage
);

/** Read a @ref pineappl_grid from a file with name `filename`. */
pineappl_grid_ptr pineappl_grid_read(char const* filename);

/** Delete a grid previously created with @ref pineappl_grid_new. */
void pineappl_grid_delete(pineappl_grid_ptr grid);

/** Return the @ref pineappl_subgrid_format that `grid` was created with. */
pineappl_subgrid_format pineappl_grid_get_subgrid_format(pineappl_grid_ptr grid);

/** Return the number of subgrids in `grid`. */
unsigned pineappl_grid_get_subgrids(pineappl_grid_ptr grid);

/** Write the subgrid parameters of `grid` into `subgrid_params`. */
void pineappl_grid_get_subgrid_params(pineappl_grid_ptr grid, int* subgrid_params);
...
void pineappl_grid_scale(pineappl_grid_ptr grid, double factor);

/** Write `grid` to a file with name `filename`. */
void pineappl_grid_write(pineappl_grid_ptr grid, char const* filename);
 ...
 * last element of `pdfs`. Quarks PDF values must be written into `pdfs[i]`, where `i` is calculated
 * as follows: \f$ i = \mathrm{PDG value} + 6 \f$. The value of the gluon PDF must be written in
 * between the quark PDFs, at index `6`.
 */
...
/**
 * Perform a convolution of the APPLgrid given as `grid` with the PDFs `pdf1` and `pdf2`. The value
 * `grid_mask` must either be `NULL` or an array as long as the luminosity function, which can be
 * used to selectively include (set the corresponding value in `grid_mask` to anything non-zero) or
 * exclude (zero) partonic initial states. The factors `scale_ren`, `scale_fac`, and `scale_energy`
 * can be used to scale the renormalisation scale, the factorisation scale, and the energy from the
 * their default (the value the grid was generated with) value. Results must be an array as large as
 * there are bins, and will contain the cross sections for each bin.
 */
