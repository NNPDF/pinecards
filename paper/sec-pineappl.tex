...
Note that in this section we have omitted the dependence of the weights, observable and scales on the specific kinematics for
which they are computed. Indeed, beyond LO,
different kinematic contributions have to be considered (in ref.~\cite{Bertone:2014zva} they are labelled with an index $\alpha$, see
eq.\ 12 therein). Their consistent treatment must be ensured by the interface between
\textsc{PineAPPL} and the specific cross-section computation at hand\footnote{
    In the FKS subtraction scheme~\cite{Frixione:1995ms,Frixione:1997np} employed in \textsc{mg5\_aMC@NLO}
    maximally one kind of kinematics for each counterterm (soft, collinear, and soft-collinear) is needed, but this
    is not the general case, for example in the Catani--Seymour subtraction~\cite{Catani:1996jh} different dipoles may have different phase spaces and therefore different scales.}.
However, \textsc{PineAPPL} remains completely blind to this fact, as all weights are filled into a given grid
identified only by the indices $k, l, m, n$.
...
Using the bin centres and their values one already has a straightforward implementation of eq.~\eqref{eq:weight-map}, but given a finite number of bins it usually yields an insufficient approximation for the cross section.
Increasing the number of bins improves the accuracy, but it also increases the space requirements.
Interpolation methods offer more accuracy with the same number of bins.
...
This reflects our knowledge of PDFs, which behave differently in those regions, and thereby increases the accuracy of the interpolation.
...
To illustrate the filling step we give an example in figure~\ref{fig:grid}, where, for simplicity, we assume we have chosen a static scale, so that we do not need to interpolate $\tau$ direction, and where we also limited the number of grid points to $N_y = 10$.
